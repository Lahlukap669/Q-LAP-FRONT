  ...
  <div class="min-h-screen visualization-bg">
    ...
    <main class="container mx-auto px-4 py-6">
      ...
      <div class="text-center mb-6">
        ...
        <p class="text-white/90">Moja analiza napredka</p>
      </div>
      ...
      <div v-if="selectedVisualizationType === 'Testi'" class="charts-container">
        <!-- No Data -->
        <div v-if="!testAnalytics || !testAnalytics.tests || testAnalytics.tests.length === 0" class="no-data text-center py-16">
          ...
          <p class="text-white/80">Trenutno ni na voljo testnih podatkov.</p>
        </div>
      ...
      <div v-else-if="selectedVisualizationType === 'Gibalne sposobnosti'" class="charts-container">
        ...
        <div v-if="!motorAbilityAnalytics || !motorAbilityAnalytics.tests || motorAbilityAnalytics.tests.length === 0" class="no-data text-center py-16">
          ...
          <h3 class="text-2xl font-bold text-white mb-4">Ni podatkov</h3>
          <p class="text-white/80">Trenutno ni na voljo podatkov o motoričnih sposobnostih.</p>
        </div>
...
export default {
  ...
  computed: {
    userFullName() {
      ...
      return `${user.first_name} ${user.last_name}` || 'Athlete'
    }
  },
  methods: {
    async fetchTestAnalytics() {
      ...
      try {
        console.log('Kličem TEST API za atleta')

        const response = await apiClient.get('/users/athlete/get-test-analytics')
    ...
    async fetchMotorAbilityAnalytics() {
      ...
      try {
        console.log('Kličem MOTOR ABILITY API za atleta')

        const response = await apiClient.get('/users/athlete/get-motor-ability-analytics')
  ...
  mounted() {
    ...
    // Automatically fetch test analytics on component mount
